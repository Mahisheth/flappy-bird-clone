...
function preload() {
  ...
  this.load.image("road", "./pictures/road.png");
  this.load.image("column", "./pictures/column.png");
  this.load.spritesheet("bird", "./pictures/bird.png", {
    frameWidth: 64,
    frameHeight: 96,
  });
}
...
function update() {
  ...
  if(bird.y > 750) {
    ...
    messageToPlayer.text = `Yay! you won!`;
  }
  if (hasLanded || hasBumped) {
  ...
  if ((hasLanded || hasBumped) && cursors.space.isDown) {
  this.scene.restart();
  hasLanded = false;
  hasBumped = false;
  isGameStarted = false;
